...
interface  PolicyBenefitsRepository:CrudRepository<PolicyBenefits,String> {
    ...
    @Query("select  DISTINCT policyId,polEffDate,BenefitId,BenefitTypeId,benefitName\n" +
            "\n" +
            "from IassistSubBenefits \n" +
            "WHERE\n" +
            " \n" +
            " (CAST(case when LIMIT='0.00' then 0 else LIMIT END  AS numeric)>0 OR ISNULL(full_Refund,full_Refund2)='FULL REFUND')\n" +
            "AND  policyId=:policyId AND poleffDate=:polEffDate and mainBenefit =:benefitId \n")
